...
private:
    ...
    void logFaceEvent(const QString& eventType,
                      const QString& level,
                      const QString& result,
                      const QString& reasonCode,
                      const QString& message,
                      std::optional<int> uid = std::nullopt,
                      std::optional<float> riskScore = std::nullopt);
    ...
    float m_similarityThreshold  = 0.60f;
    ...
    QString m_traceId;
    QString m_lastAuditEventType;
    QDateTime m_lastAuditTime;
};
